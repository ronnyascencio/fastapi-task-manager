...
async def get_task_id(
    ...
    task_model = (
        ...
        .filter(Tasks.owner_id == user.get("id"))
        .first()
    )
...
async def get_task_by_owner_id(
    user: user_dependency, db: db_dependency, owner_id: int = Path(gt=0)
):
    if user is None:
        raise HTTPException(status_code=401, detail="authentication failed")
...
async def post_task(
    user: user_dependency, db: db_dependency, task_request: TaskRequest
):
    if user is None:
        raise HTTPException(status_code=401, detail="authentication failed")
...
async def update_task_id(
    user: user_dependency, db: db_dependency, task_id: int, task_request: TaskRequest
):
    if user is None:
        raise HTTPException(status_code=401, detail="authentication failed")
    task_model = (
...
async def delete_task_by_id(user: user_dependency, db: db_dependency, task_id: int):
    ...
    if task_model is None:
        raise HTTPException(status_code=404, detail="task not found")
    db.query(Tasks).filter(Tasks.id == task_id).filter(
...
async def delete_task_by_title(
    user: user_dependency, db: db_dependency, task_title: str
):
    if user is None:
        raise HTTPException(status_code=401, detail="authentication failed")
    task_model = (
        db.query(Tasks)
        .filter(Tasks.title == task_title)
        .filter(Tasks.owner_id == user.get("id"))
        .first()
    )
    ...
    db.query(Tasks).filter(Tasks.title == task_title).filter(
        Tasks.owner_id == user.get("id")
    ).delete()
    db.commit()
